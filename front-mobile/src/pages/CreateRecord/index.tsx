...
const CreateRecord = () => {
    ...
    const [allGames, setAllGame] = useState<Game[]>([])
    const [filtredGames, setFiltredGame] = useState<Game[]>([])
    ...
    useEffect(() => {
        ...
        .catch(()=>{
            Alert.alert('Erro ao listar jogos!');            
        })
    }, [])
    ...
    const handleChangePlatform = (selectPlatform: GamePlatform) => {
        ...
        setFiltredGame(gamesByPlatform)
    }

    const handleSubmit = () => {
        const payload = {
            name, age, gameId: selectedGame
        };
